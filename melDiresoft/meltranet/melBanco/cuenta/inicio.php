<?php include '../../meltranet.php';
	...
	$laLis = lisIni('cuentaRec');	
	
	$lcPro = "CALL cuentaRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear una cuenta</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL cuentaRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idCuenta" value="<?php iMostrar($laMel["idCuenta"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>"><div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cNombre"]);?></div><div style="width:220px;" class="enviarFila"><?php cMostrar($laMel["cBanco"]);?></div><div style="width:100px; text-align:right;" class="enviarFila"><?php yMostrar($laMel["ySaldo"]);?></div></button>
			</div>
		</form><?php
	}
