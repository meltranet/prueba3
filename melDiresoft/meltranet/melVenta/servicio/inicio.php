<?php include '../../meltranet.php';
	...
	$laLis = lisIni('servicioRec');	
	
	$lcPro = "CALL servicioRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear un servicio</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL servicioRec(".idbVal($laLis['iRec']).",".
		...
		<form method="post" action="ver/inicio.php">
	    	...
	    	<div title="<?php cMostrar($laReg["cNombre"]);?>" class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>">
					<div style="width:600px;" class="enviarFila"><?php cMostrar($laReg["cNombre"]);?></div>
					<div style="width:80px; text-align:right;" class="enviarFila"><?php yMostrar($laReg["yPrecio"]);?></div>
				</button>
			</div>
		</form><?php
	}
