<?php include '../../meltranet.php';
	...
	$laLis = lisIni('promotorRec');	
	
	$lcPro = "CALL promotorRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear un promotor</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL promotorRec(".idbVal($laLis['iRec']).",".
	...
	while ($laReg = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idPromotor" value="<?php iMostrar($laReg["idPromotor"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>">
					<div style="width:320px;" class="enviarFila"><?php cMostrar($laReg["cNombre"]);?></div>
					<div style="width:320px;" class="enviarFila"><?php yMostrar($laReg["nComision"]);?>%</div>
				</button>
			</div>
		</form><?php
	}
