<?php include '../../meltranet.php';
	...
	$laLis = lisIni('cotizacionRec');	
	
	$lcPro = "CALL cotizacionRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear una cotizaci&oacute;n</a></li>
			<li class="menuH"><span class="separadorH">&#x2022;</span><li class="menuH"><a class="menuH" href="informe" target="_self">informes de cotizaciones</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL cotizacionRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idCotizacion" value="<?php iMostrar($laMel["idCotizacion"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>">
					...
					<div style="width:360px;" class="enviarFila"><?php cMostrar($laMel["cClienteNombre"],40);?></div>
					<div style="width:120px; text-align:right;" class="enviarFila"><?php yMostrar($laMel["yTotal"]);?></div>
				</button>
			</div>
		</form><?php
	}
