<?php include '../../../../meltranet.php';
	...
	if (postExi('iImprimir')==true) {
		$lcPro = "CALL cotizacioninfRec(1,".
			cdbPost('cDesde').",".
			cdbPost('cHasta').idbMel();
		$laMel = dbResFilIni($lcPro);
	} 
		
	$lcPro = "CALL cotizacioninfRec(2,".
		cdbPost('cDesde').",".
		cdbPost('cHasta').idbMel();
...
<body>
    ...
    <div style="margin-top:12px; padding-left:12px; margin-right:12px; width:740px;">
		<div class="listaFilaTitulo">
			...
			<div style="width:70px; text-align:left; font-size:11px;" class="listaCeldaTitulo1">Factura</div>
			<div style="width:80px; text-align:left; font-size:11px;" class="listaCeldaTitulo1">Fecha</div>
			<div style="width:430px; text-align:left; font-size:11px;" class="listaCeldaTitulo1">Cliente</div>
			<div style="width:70px; text-align:right; font-size:11px;" class="listaCeldaTitulo2">Total</div>
		...
		function inf0 ($taInf) {?>
			<div>
				...
				<div style="width:430px; text-align:left;" class="listaCeldaNormal1"><?php cMostrar($taInf['cClienteNombre'],40);?></div>
    			<div style="width:70px; text-align:right;" class="listaCeldaNormal2"><?php yMostrar($taInf["yTotal"]);?></div>
	   	        <div class="listaCeldaCorte"></div>
			</div><?php
		}
		function inf1 ($taInf) {?>
