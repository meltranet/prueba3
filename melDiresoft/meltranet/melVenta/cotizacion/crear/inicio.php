<?php include '../../../meltranet.php';
	...
	if (postExi('idCliente')==true) {
		$lcPro = "CALL cotizacionIns(0,".
			idbPost('idCliente').idbMel();
		$laMel = dbResFilIni($lcPro);
		if ($laMel["iError"] == 0) {cargarPag('servicio/inicio.php');}
	}
	
	$laLis = lisIni('clienteRec1');	

	$lcPro = "CALL clienteRec1(0,0,0,'',''".idbMel();
...
<body>
    <div class="menuN">
		<ul class="menuN">
			...
			<li class="menuN"><a class="menuN" href="../" target="ifmMeltranet">Cotizaciones</a><span class="separadorN">/</span></li>
		</ul>
    </div>
	<h1>Crear una Cotizaci&oacute;n</h1>
    <div class="menuH">
		<ul class="menuH">
	...
	<div style="padding-top:6px;">
		<div style="width:370px; float:left; border-right:none;" class="lista">
			...
			$lcPro = "CALL clienteRec1(".idbVal($laLis['iRec']).",".
			...
			while ($laReg = dbResFilCar($loReg)) {
				...
				<form method="post">
    		    	<input type="hidden" name="idCliente" value="<?php iMostrar($laReg["idCliente"]);?>">
	    			<div class="<?php cMostrar($lcRegCla);?>">
            			<button type="submit" class="enviarFila0" title="<?php cMostrar($laReg["cNombre"]);?>"><div style="width:360px;" class="enviarFila"><?php cMostrar($laReg["cNombre"],40);?></div></button>
					</div>
				</form><?php
			}
			dbResRem($loReg);
		...
		<div style="width:364px; float:right;">
			Seleccione un cliente para la cotizaci&oacute;n.
		</div>
