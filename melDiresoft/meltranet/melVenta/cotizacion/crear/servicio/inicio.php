<?php include '../../../../meltranet.php';
	...
	if (postExi('eNota')==true) {
		...
		if (postExi('idServicio')==true) {
			$lcPro = "CALL cotizacionServicioIns(0,".
				idbPost('idServicio').idbMel();
			$laMel = dbResFilIni($lcPro);
		}	
	
		$lcPro = "CALL cotizacionIns1(0,".
			edbPost('eNota').idbMel();
		$laMel = dbResFilIni($lcPro);
	}
...
<body>
	...
	<div style="padding-top:6px;">
		...
		<div style="width:364px; float:right;"><?php
			$lcPro = "CALL cotizacionServicioRec(0,0".idbMel();
			...
			if ($liDet==0) {cMostrar("Seleccione un servicio para agregar a la factura.");}
				...
				$lcOverflow = ifInmediato(($laMel['iPx']>222), "height:222px; overflow:auto;", ""); ?>
				<div style="border-bottom:1px solid #C6D4E0; font-size:10px; <?php cMostrar($lcOverflow);?> "><?php
					...
					while ($laDet = dbResFilCar($loDet)) {
						...
						<form action="servicio.php" method="post">
	        				<input type="hidden" name="idCotizacionServicio" value="<?php iMostrar($laDet["idCotizacionServicio"]);?>">
				...
				<form method="post">
					...
					<div style="padding:6px; text-align:right;"><button type="submit">Crear cotizaci&oacute;n</button></div>
				</form><?php
			}?>
		</div>
