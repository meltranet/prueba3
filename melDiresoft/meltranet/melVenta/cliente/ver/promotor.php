	    ...
	    <div style="width:280px; padding-top:3px;">
			<div style="border-bottom:1px solid #C6D4E0; padding:6px;">
				<div style=" padding-bottom:3px;">
					...
					<select name="idPromotor" style="width:260px; background-color:#FFFFFF; font-family:Arial; font-size:13px; margin:0px; padding:3px; border:none;"><?php
						while ($laSel = dbResFilCar($loSel)) { ?>
							<option value="<?php iMostrar($laSel['idPromotor']); ?>" <?php if($laSel['idPromotor']==$laMel['idPromotor']) {cMostrar("selected");}?> ><?php cMostrar($laSel['cNombre']); ?></option><?php 
						} ?>
					</select>
		        </div>
			</div>
			...
			<div style="padding:6px;"><button type="submit">Cambiar promotor</button></div>
		</div>
	</form>
</body>
</html>
