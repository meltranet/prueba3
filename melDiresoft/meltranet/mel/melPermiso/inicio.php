<?php 
	...
	$laLis = lisIni('melPermisoRec');	
	
	$lcPro = "CALL melPermisoRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear un permiso</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL melPermisoRec(".idbVal($laLis['iRec']).",".
	...
	while ($laReg = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idMelPermiso" value="<?php iMostrar($laReg["idMelPermiso"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>"><div style="width:320px;" class="enviarFila"><?php cMostrar($laReg["cNombre"]);?></div><div style="width:320px;" class="enviarFila"><?php cMostrar($laReg["cDescripcion"]);?></div></button>
			</div>
		</form><?php
	}
