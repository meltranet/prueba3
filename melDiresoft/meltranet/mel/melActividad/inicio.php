<?php 
	...
	$laLis = lisIni('melActividadRec');
	
	$lcPro = "CALL melActividadRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear una actividad</a></li>
			<li class="menuH"><span class="separadorH">&#x2022;</span><li class="menuH"><a class="menuH" href="informe" target="_self">informes de actividades</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL melActividadRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		$liReg = $liReg + 1;
		$lcRegCla = ifInmediato(($lcRegCla == 'listaFilaNormal0'), 'listaFilaNormal1', 'listaFilaNormal0');
		if ($liReg==1) {$liRegPri = $laMel["iRegPri"]; $liRegUlt = $laMel["iRegUlt"]; $liRegTot = $laMel["iRegTot"];}
		if ($laMel["iEstado"]<1) {$lcBotCla="enviarFila1";} else {$lcBotCla="enviarFila0";}?>
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idMelActividad" value="<?php iMostrar($laMel["idMelActividad"]);?>">
	    	<div style="margin:0px; padding: 0px;" class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>">
                	<div style="width:80px;" class="enviarFila"><?php cMostrar($laMel["cNumero"]);?></div>
                    <div style="width:100px;" class="enviarFila"><?php dMostrar($laMel["dFecha"]);?></div>
                    <div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cDescripcion"],40);?></div>
                    <div style="width:120px;" class="enviarFila"><?php dMostrar($laMel["dRealizadaFecha"]);?></div>
				</button>
			</div>
		</form><?php
	}
