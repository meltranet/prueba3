<?php 
	...
	if (postExi('iImprimir')==true) {
		$lcPro = "CALL melActividadInfRec(1,".
			cdbPost('cDesde').",".
			cdbPost('cHasta').idbMel();
		$laMel = dbResFilIni($lcPro);
	} else {exit;}
		
	$lcPro = "CALL melActividadInfRec(2,".
		cdbPost('cDesde').",".
		cdbPost('cHasta').idbMel();
...
<body>
    ...
    <div style="margin-top:12px; padding-left:12px; margin-right:12px; width:740px;">
		<div class="listaFilaTitulo">
			...
			<div style="width:70px; text-align:left; font-size:11px;" class="listaCeldaTitulo1">Actividad</div>
			<div style="width:80px; text-align:left; font-size:11px;" class="listaCeldaTitulo1">Fecha</div>
			<div style="width:240px; text-align:left; font-size:11px;" class="listaCeldaTitulo1">Cliente</div>
			<div style="width:120px; text-align:left; font-size:11px;" class="listaCeldaTitulo1">Incidente</div>
			<div style="width:70px; text-align:center; font-size:11px;" class="listaCeldaTitulo1">Realizada</div>
			<div style="width:70px; text-align:right; font-size:11px;" class="listaCeldaTitulo2">Duraci&oacute;n</div>
		...
		function inf0 ($taInf) {?>
			<div>
				...
				<div style="width:240px; text-align:left;" class="listaCeldaNormal1"><?php cMostrar($taInf['cClienteNombre'],28);?></div>
				<div style="width:120px; text-align:left;" class="listaCeldaNormal1"><?php cMostrar($taInf['cMelIncidenteNumero']);?></div>
				<div style="width:70px; text-align:center;" class="listaCeldaNormal1"><?php dMostrar($taInf['dRealizadaFecha']);?></div>
    			<div style="width:70px; text-align:right;" class="listaCeldaNormal2"><?php cMostrar($taInf["cDuracion"]);?></div>
	   	        <div class="listaCeldaCorte"></div>
			</div><?php
		}
		function inf1 ($taInf) {?>
			...
			<div>
				...
				<div style="width:596px; text-align:left;" class="listaCeldaNormal1">TOTAL (horas)</div>
				<div style="width:70px; text-align:right;" class="listaCeldaNormal2"><?php cMostrar($taInf["cDuracion"]);?></div>
	   	        <div class="listaCeldaCorte"></div>
			</div><?php
		}
