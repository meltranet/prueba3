<?php 
	...
	if (postExi('cMotivo')==true) {
		$lcPro = "CALL melActividadAct1(1,".
			idbPost('idMelActividad').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
		
		if ($laMel["iError"] == 0) {cargarPag('../inicio.php');}
	} else {
		$lcPro = "CALL melActividadAct1(0,".
			idbPost('idMelActividad').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
	}
...
<body>
	...
	<div style="padding-top:6px; width:280px;">
		<form method="post">
			...
			<div style="padding:6px;"><button type="submit">Anular actividad</button></div>
		</form>
	</div>
</body>
</html>
