<?php 
	...
	if (postExi('idMelIncidente')==true) {
		$lcPro = "CALL melActividadIns(0,".
			idbPost('idMelIncidente').idbMel();
		$laMel = dbResFilIni($lcPro);
		if ($laMel["iError"] == 0) {cargarPag('descripcion.php');}
	}
	
	$laLis = lisIni('melIncidenteRec1');	

	$lcPro = "CALL melIncidenteRec1(0,0,0,'',''".idbMel();
...
<body>
	...
	<div style="padding-top:6px;">
		<div style="width:370px; float:left; border-right:none;" class="lista">
			<form method="post">
				<div style="display:inline;">
					<select style="width:260px;" name="cExpSen"><?php
						$lcSen = ''; $lcOpcion = '';
			...
			$lcPro = "CALL melIncidenteRec1(".idbVal($laLis['iRec']).",".
			...
			while ($laReg = dbResFilCar($loReg)) {
				...
				<form method="post">
    		    	<input type="hidden" name="idMelIncidente" value="<?php iMostrar($laReg["idMelIncidente"]);?>">
	    			<div class="<?php cMostrar($lcRegCla);?>" title="<?php cMostrar($laReg["cDescripcion"]);?>">
            			<button type="submit" class="enviarFila0">
							<div style="width:74px;" class="enviarFila"><?php cMostrar($laReg["cNumero"]);?></div>
							<div style="width:94px;" class="enviarFila"><?php dMostrar($laReg["dFecha"]);?></div>
							<div style="width:186px;" class="enviarFila"><?php cMostrar($laReg["cDescripcion"],24);?></div>
						</button>
					</div>
				</form><?php
			}
		...
		<div style="width:364px; float:right;">
			Seleccione un incidente para la actividad.
		</div>
