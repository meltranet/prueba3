<?php include '../../../meltranet.php';
	...
	if (postExi('idMelModulo')==true) {
		$lcPro = "CALL melCuentaModuloIns(1,0,".
			idbPost('idMelModulo').idbMel();
		$laMel = dbResFilIni($lcPro);
		if ($laMel["iError"] == 0) {cargarPag('../inicio.php');}
	} else {
		$lcPro = "CALL melCuentaModuloIns(0,0,".
			idbPost('idMelModulo').idbMel();
		$laMel = dbResFilIni($lcPro);
	}

	$laLis = lisIni('melModuloRec1');	

	$lcPro = "CALL melModuloRec1(0,0,0,'',''".idbMel();
...
<body>
    <div class="menuN">
		<ul class="menuN">
			...
			<li class="menuN"><a class="menuN" href="../" target="ifmMeltranet"><?php cMostrar($laMel["cNombre"]);?></a><span class="separadorN">/</span></li>
		</ul>
    </div>
	<h1>Agregar un Enlace R&aacute;pido</h1>
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="../" target="ifmMeltranet">cancelar</a></li>
		</ul>
    </div><?php 
	if ($laMel["iError"] == 1){mensaje($laMel["iMensaje"], $laMel["cMensaje"], $laMel["vMensaje"]);}?>
	<div style="padding-top:6px;">    
		<div style="width:370px; float:left; border-right:none;" class="lista">
			...
			$lcPro = "CALL melModuloRec1(".idbVal($laLis['iRec']).",".
			...
			while ($laReg = dbResFilCar($loReg)) {
				...
				<form method="post">
    		    	<input type="hidden" name="idMelModulo" value="<?php iMostrar($laReg["idMelModulo"]);?>">
	    			<div class="<?php cMostrar($lcRegCla);?>">
            			<button type="submit" class="enviarFila0"><div style="width:360px;" class="enviarFila"><?php cMostrar($laReg["cNombre"]);?></div></button>
					</div>
				</form><?php
			}
			dbResRem($loReg);
		...
		<div style="width:364px; float:right;">
			Seleccione un monulo para agregar.
		</div>
