<?php 
	...
	$laLis = lisIni('melClasificacionRec');	
	
	$lcPro = "CALL melClasificacionRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear una clasificaci&oacute;n</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL melClasificacionRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idMelClasificacion" value="<?php iMostrar($laMel["idMelClasificacion"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>"><div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cNombre"]);?></div><div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cProceso"]);?></div></button>
			</div>
		</form><?php
	}
