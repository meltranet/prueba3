<?php 
	...
	$laLis = lisIni('melCierreRec');
	
	$lcPro = "CALL melCierreRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear un cierre</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL melCierreRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idMelCierre" value="<?php iMostrar($laMel["idMelCierre"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>">
                    ...
                    <div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cConcepto"],40);?></div>
                    <div style="width:120px;" class="enviarFila"><?php dMostrar($laMel["dCierre"]);?></div>
				</button>
			</div>
		</form><?php
	}
