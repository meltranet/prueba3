<?php 
	...
	if (postExi('cMotivo')==true) {
		$lcPro = "CALL melIncidenteAct1(1,".
			idbPost('idMelIncidente').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
		
		if ($laMel["iError"] == 0) {cargarPag('../inicio.php');}
	} else {
		$lcPro = "CALL melIncidenteAct1(0,".
			idbPost('idMelIncidente').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
	}
...
<body>
	...
	<div style="padding-top:6px; width:280px;">
		<form method="post">
			...
			<div style="padding:6px;"><button type="submit">Anular incidente</button></div>
		</form>
	</div>
</body>
</html>
