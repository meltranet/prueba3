<?php 
	...
	$laLis = lisIni('melIncidenteRec');
	
	$lcPro = "CALL melIncidenteRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
    ...
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear un incidente</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL melIncidenteRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idMelIncidente" value="<?php iMostrar($laMel["idMelIncidente"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>">
                    ...
                    <div style="width:120px;" class="enviarFila"><?php dMostrar($laMel["dOcurridoFecha"]);?></div>
				</button>
			</div>
		</form><?php
	}
