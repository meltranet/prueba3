<?php 
	...
	$laLis = lisIni('melCuentaRec');	
	
	$lcPro = "CALL melCuentaRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<?php
	...
	$lcPro = "CALL melCuentaRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idMelCuenta" value="<?php iMostrar($laMel["idMelCuenta"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>"><div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cNombre"]);?></div><div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cCompleto"]);?></div></button>
			</div>
		</form><?php
	}
