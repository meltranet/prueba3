<?php include '../../meltranet.php';
	...
	$laLis = lisIni('retencionvenRec');	
	
	$lcPro = "CALL retencionvenRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
	...
	<h1>Retenciones en Ventas</h1>
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="crear" target="_self">crear una retenci&oacute;n</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL retencionvenRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idRetencionven" value="<?php iMostrar($laMel["idRetencionven"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>"><div style="width:80px;" class="enviarFila"><?php cMostrar($laMel["cNumero"]);?></div><div style="width:100px;" class="enviarFila"><?php dMostrar($laMel["dFecha"]);?></div><div style="width:320px;" class="enviarFila"><?php cMostrar($laMel["cClienteNombre"],40);?></div><div style="width:120px; text-align:right;" class="enviarFila"><?php yMostrar($laMel["yMonto"]);?></div></button>
			</div>
		</form><?php
	}
