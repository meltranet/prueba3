<?php 
	...
	if (postExi('cMotivo')==true) {
		$lcPro = "CALL retencionvenAct1(1,".
			idbPost('idRetencionven').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
		
		if ($laMel["iError"] == 0) {cargarPag('../inicio.php');}
	} else {
		$lcPro = "CALL retencionvenAct1(0,".
			idbPost('idRetencionven').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
	}
...
<body>
	...
	<h1>Anular la Retenci&oacute;n</h1>
    <div class="menuH">
		<ul class="menuH">
	...
	<div style="padding-top:6px; width:280px;">
		<form method="post">
    		<input name="idRetencionven" value="<?php cMostrar($laMel['idRetencionven']); ?>" type="hidden">
			...
			<div style="padding:6px;"><button type="submit">Anular retenci&oacute;n</button></div>
		</form>
	</div>
</body>
</html>
