<?php include '../../../../meltranet.php';
	...
	if (postExi('eNota')==true) {
		...
		if (postExi('idFactura')==true) {
			$lcPro = "CALL retencionvenFacturaIns(0,".
				idbPost('idFactura').idbMel();
			$laMel = dbResFilIni($lcPro);
		}	
	
		$lcPro = "CALL retencionvenIns2(0,".
			edbPost('eNota').idbMel();
		$laMel = dbResFilIni($lcPro);
	}
...
<body>
	...
	<h1><?php cMostrar($laMel['cDescripcion']);?></h1>
    <div class="menuH">
		<ul class="menuH">
	...
	<div style="padding-top:6px;">
		...
		<div style="width:364px; float:right;"><?php
			$lcPro = "CALL retencionvenFacturaRec(0,0".idbMel();
			...
			if ($liDet==0) {cMostrar("Seleccione una factura para ser agregada.");}
		    	...
		    	<div style="background-color:#DBE7F0; color:#515F6C; border-bottom:1px solid #C6D4E0;" class="listaFilaTitulo">
					<div style="width:120px; text-align:left;" class="listaCeldaTitulo0">Facturas</div>
					<div style="width:90px; text-align:right;" class="listaCeldaTitulo1">Monto Base</div>
					<div style="width:90px; text-align:right;" class="listaCeldaTitulo1">Monto Retenido</div>
					<div class="listaCeldaTitulo2">&nbsp;</div>
		  	    	<div class="listaCeldaCorte"></div>
				</div>
				<div style="border-bottom:1px solid #C6D4E0; font-size:10px;"><?php
					$lcRegCla = ''; 
					while ($laDet = dbResFilCar($loDet)) {
						$lcRegCla = ifInmediato(($lcRegCla == 'listaFilaNormal0'), 'listaFilaNormal1', 'listaFilaNormal0'); ?>
                       	<form action="factura.php" method="post">
	        				<input type="hidden" name="idRetencionvenFactura" value="<?php iMostrar($laDet["idRetencionvenFactura"]);?>">
		    				<div class="<?php cMostrar($lcRegCla);?>">
								<div style="width:120px;" class="listaCeldaNormal0"><?php cMostrar($laDet["cFacturaNumero"]);?></div>
			            		<div style="width:90px; text-align:right;" class="listaCeldaNormal1"><?php yMostrar($laDet["yBase"]);?></div>
			            		<div style="width:90px; text-align:right;" class="listaCeldaNormal1""><?php yMostrar($laDet["yMonto"]);?></div>
				...
				<form method="post">
					...
					<div style="padding:6px; text-align:right;"><button type="submit">Crear retenci&oacute;n</button></div>
				</form><?php
			}?>
		</div>
