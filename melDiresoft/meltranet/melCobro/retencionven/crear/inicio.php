<?php include '../../../meltranet.php';
	...
	if (postExi('idCliente')==true) {
		$lcPro = "CALL retencionvenIns(0,".
			idbPost('idCliente').idbMel();
		$laMel = dbResFilIni($lcPro);
		
		if ($laMel["iError"] == 0) {cargarPag('cliente/inicio.php');}
	} 

	$laLis = lisIni('clienteRec2');	

	$lcPro = "CALL clienteRec2(0,0,0,'',''".idbMel();
...
<body>
	...
	<h1>Crear una Retenci&oacute;n</h1>
    <div class="menuH">
		<ul class="menuH">
	...
	<div style="padding-top:6px;">
		<div style="width:370px; float:left; border-right:none;" class="lista">
			...
			$lcPro = "CALL clienteRec2(".idbVal($laLis['iRec']).",".
			...
			while ($laReg = dbResFilCar($loReg)) {
				...
				<form method="post">
	    			...
	    			<div title="<?php cMostrar($laReg["cNombre"]);?>" class="<?php cMostrar($lcRegCla);?>">
            			<button type="submit" class="enviarFila0"><div style="width:286px;" class="enviarFila"><?php cMostrar($laReg["cNombre"],32);?></div><div style="width:72px; text-align:right;" class="enviarFila"><?php cMostrar($laReg["yPrecio"]);?></div></button>
					</div>
				</form><?php
			}
			dbResRem($loReg);
		...
		<div style="width:364px; float:right;">Seleccione un cliente de la retenci&oacute;n.</div>
