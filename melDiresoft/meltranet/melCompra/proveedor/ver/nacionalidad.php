<?php include '../../../meltranet.php';
	...
	if (postExi('cMotivo')==true) {
		$lcPro = "CALL proveedorAct08(1,".
			idbPost('idProveedor').",".
			cdbPost('cNacionalidad').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
		
		if ($laMel["iError"] == 0) {cargarPag('inicio.php');}
	} else {
		$lcPro = "CALL proveedorAct08(0,".
			idbPost('idProveedor').",".
			cdbPost('cNacionalidad').",".
			cdbPost('cMotivo').idbMel();
		$laMel = dbResFilIni($lcPro);
	    ...
	    <div style="width:280px; padding-top:3px;">
			<div style="border-bottom:1px solid #C6D4E0; padding:6px;">
			    ...
			    <div style="padding:3px; border:1px solid #C6D4E0;">
		            <input name="cNacionalidad" style="width:260px; font-family:Arial; font-size:13px; margin:0px; padding:0px; border:none; background:none;"
			        	value="<?php cMostrar($laMel['cNacionalidad']); ?>" maxlength="40">
				</div>
			</div>
			<div style="border-bottom:1px solid #C6D4E0; padding:6px;">
				<div style="padding-bottom:3px;">
			...
			<div style="padding:6px;"><button type="submit">Cambiar nacionalidad</button></div>
		</div>
	</form>
</body>
</html>
