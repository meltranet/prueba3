<?php include '../../meltranet.php';
	...
	$laLis = lisIni('retencionislrRec');	
	
	$lcPro = "CALL retencionislrRec(".idbVal(0).",".
		idbVal(0).",".
		idbVal(0).",".
		cdbVal('').",".
		cdbVal('').",".
...
<body>
	...
	<h1>Retenciones de I.S.L.R.</h1>
    <div class="menuH">
		<ul class="menuH">
			<li class="menuH"><a class="menuH" href="informe" target="_self">informes de retenciones</a></li>
		</ul>
    </div>
...
<?php
	...
	$lcPro = "CALL retencionislrRec(".idbVal($laLis['iRec']).",".
	...
	while ($laMel = dbResFilCar($loReg)) {
		...
		<form method="post" action="ver/inicio.php">
        	<input type="hidden" name="idRetencionislr" value="<?php iMostrar($laMel["idRetencionislr"]);?>">
	    	<div class="<?php cMostrar($lcRegCla);?>">
            	<button type="submit" class="<?php cMostrar($lcBotCla);?>">
					...
					<div style="width:120px; text-align:right;" class="enviarFila"><?php yMostrar($laMel["yMonto"]);?></div>
				</button>
			</div>
		</form><?php
	}
